...
class manager {
	...
	function getObjects($pattern, $where, $currentPage, $objPerPage, $categories = 0, $cat = NULL) {
		...
		if(empty($return)) $return = "<div id=\"display_error\">Brak obiektÛw.</div>";
	...
	//Wyúwietlanie obiektu (pojedynczy)
	...
	//Licznik obiektÛw (glowan=0/poczekalnia=1/archiwum=2)
	...
	function searchObjects($pattern, $search, $currentPage, $objPerPage) {
		...
		if(empty($return)) $return = "<div id=\"display_error\">Nie znaleziono obiektÛw spe≥niajπcych kryteria wyszukiwania.</div>";
	...
	function getFavorites($pattern, $profileId) {
		...
		if(!empty($profileId) && is_numeric($profileId)) {
			...
			//Modu≥ wczytuje ulubione obiekty uøytkownika.
	...
	function uploadImage($max_width, $max_file_size, $owner, $watermark) {
		...
		if(isset($_POST['send_img'])) {	
			...
			if(empty($_POST['title']) || !$_FILES['image']['name']) { $this->msg('Uzupe≥nij wszystkie pola!',1); }
					...
					if(filesize($_FILES['image']['tmp_name']) <= $max_file_size*1024) {
						...
						$image_info = @getimagesize($_FILES['image']['tmp_name']);
							...
							if(array_search($file_ex['extension'],$file_types) && array_search($image_info['mime'],$file_mimes)) {
								...
								if($overwrite == 0 && file_exists($dir.$file_name.".".$file_ex['extension'])) $this->msg('Taki plik juø istnieje.',1);

								if(!move_uploaded_file($_FILES['image']['tmp_name'],$dir.$file_name.".".$file_ex['extension'])) $this->msg('Wgrywanie pliku nie powiod≥o siÍ.',1);
									...
									$this->msg('Obiekt zosta≥ pomyúlnie za≥adowany.',3);
	...
	function addMovie($owner, $type_video) {
		...
		if(isset($_POST['send_movie'])) {
			...
			if(empty($_POST['title']) || empty($_POST['src'])) { $this->msg('Uzupe≥nij wszystkie pola!',1); }
				...
				if($_SESSION['img_number'] != strtolower($_POST['question'])) $this->msg('Kod weryfikacyjny nie jest poprawny!',1);
					...
					switch($type_video) {
						...
						case 'youtube':
							...
							parse_str(parse_url($_POST['src'], PHP_URL_QUERY), $parsedLink);

							$getHeadersToVerify = get_headers("http://gdata.youtube.com/feeds/api/videos/".$parsedLink['v']);

							if(!strpos($getHeadersToVerify[0],'200')) { $this->msg('Nie moøna dodaÊ klipu wideo, poniewaø nie isnieje.',1); return false; }
					...
					if($query) $this->msg('Obiekt zosta≥ poprawnie dodany!',3);

					else $this->msg('Nie uda≥o siÍ dodaÊ obiektu.',1);
	...
	function uploadForm($rewrite, $type) {
		...
		if($type == 'img') {
			...
			echo '<form action="'.$rewrite->add.'" method="post" enctype="multipart/form-data">
                ...
                else if($type == 'imgfromurl') {
                echo '<form action="'.$rewrite->add.'" method="post">
				<label>Nazwa</label>
				<input type="text" name="title" maxlength="64" value="'.(isset($_POST['title'])?htmlspecialchars($_POST['title']):'').'" />
				<label>Adres obrazka</label>
				<input type="text" name="image_url" value="'.(isset($_POST['url'])?htmlspecialchars($_POST['url']):'').'" />
				<label>Kategoria</label>
				<select name="cat">
					'.$this->catList().'
				</select>
				...
				<input type="text" name="question">
				<br/>
				<input type="submit" name="send_imgfromurl" value="Dodaj" />
			</form>';
}
	...
	//mod_tools - zarzπdzanie obiektami przez moderatorÛw
		...
		if($user->verifyLogin()) {
				...
				if($user->userInfo('rank') <= 1) {
					...
					if($img['is_waiting'])
						...
						$mod_tools .= '<a href="#" onClick="mod_move('.$img['id'].'); return false;">Przenieú na g≥Ûwnπ</a>';

					else

						$mod_tools .= '<a href="#" onClick="mod_move('.$img['id'].'); return false;">Przenieú do poczekalni</a>';

					

					$mod_tools .= '<a href="#" onClick="mod_amove('.$img['id'].'); return false;">Przenieú do archiwum</a>';

					$mod_tools .= '<a href="#" onClick="mod_del('.$img['id'].'); return false;">UsuÒ</a>';
	...
	function comments($pattern, $page_settings) {
		...
		else echo 'Komentarze sπ wy≥πczone.';

	}
        
		function addImagefromURL($max_width, $max_file_size, $owner, $watermark) {
		$overwrite = 0;
		$dir = 'upload/';
		$file_types = array(1=>'jpg','jpeg','gif','png','JPG','JPEG','GIF','PNG');
		$file_mimes = array(1=>'image/jpg', 'image/jpeg', 'image/pjpeg', 'image/gif', 'image/png');
		...
		if(isset($_POST['send_imgfromurl'])) {	
			if(empty($_POST['title']) || empty($_POST['image_url'])) { $this->msg('Uzupe≥nij wszystkie pola!',1); }
			else { 
				if($_SESSION['img_number'] != strtolower($_POST['question'])) $this->msg('Kod weryfikacyjny nie jest poprawny!',1);
				else {
					$title = mysql_real_escape_string(htmlspecialchars($_POST['title']));
					$image = mysql_real_escape_string(htmlspecialchars($_POST['image_url']));
					if(!empty($_POST['source'])) $source = mysql_real_escape_string(htmlspecialchars($_POST['source']));
					else $source = '-----';
					$cat = mysql_real_escape_string(htmlspecialchars($_POST['cat']));
					$date = date('Y-m-d H:i:s');
					...
					if(strlen(file_get_contents($image)) <= $max_file_size*1024) {
						$file_ex = pathinfo($image);
						$image_info = @getimagesize($image);
							if(array_search($file_ex['extension'],$file_types) && array_search($image_info['mime'],$file_mimes)) {
								if($overwrite == 0 && file_exists($dir.$file_name.".".$file_ex['extension'])) $this->msg('Taki plik juø istnieje.',1);
								if(!file_put_contents($dir.$file_name.".".$file_ex['extension'],file_get_contents($image))) $this->msg('Wgrywanie pliku nie powiod≥o siÍ.',1);
								else {
									$upload_dir = $dir.$file_name.".".$file_ex['extension'];
									$file_src = $file_name.".".$file_ex['extension'];
									mysql_query("INSERT INTO `tentego_img` (`title`,`src`,`type`,`owner`,`cat`,`date`,`source`,`is_waiting`)
